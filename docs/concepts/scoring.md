...
`TF` is the naive count baseline. `TFIDF` downweights words that appear in most documents, so a term like `quality` that shows up everywhere contributes less than a rare synonym. `WFIDF` further dampens within-document frequency, matching the 2021 paper's Appendix. The `+SIMWEIGHT` variants multiply in a similarity weight derived from dictionary rank: seeds (rank 0) get weight $1/\ln 2 \approx 1.44$, rank 100 gets $1/\ln 102 \approx 0.22$. This is a **rank-based** weight — a function of a word's position in the similarity ordering, not of its raw cosine-similarity value — and is the weighting scheme adopted in several studies that build on the 2021 method. `Config.tfidf_normalize=True` L2-normalizes the tfidf vector per document. `Config.zca_whiten=True` applies ZCA whitening to decorrelate the dimension columns after scoring; see [Whiten the dimension scores](../how-to/whiten-scores.md).
