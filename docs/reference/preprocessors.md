...
dependency pairs. On the 1,393-document bakeoff it was the fastest parser
backend, with the cleanest NER output and the smallest Word2Vec-ready
vocabulary. Use when you want richer parsing than the default `"none"` without
Java; `"corenlp"` is the opt-in backend for paper-faithful results.
